...
import de.kosit.xmlmutate.schematron.ISOSchematronCompiler;
...
/**
 * Check the assertions defined within the schematron rules againgst the
 * validated state
 *
...
public class EvaluateSchematronExpectationsAction implements RunAction {
        ...
        public void run(final Mutation mutation) {
                final Pair<SchematronEnterity, ExpectedResult> schematronEnterityExpectation = mutation
                                .getConfiguration()
                                .getSchematronEnterityExpectation();
                if (schematronEnterityExpectation == null) {
                        for (final SchematronRuleExpectation expectation : mutation.getConfiguration()
                                        .getSchematronExpectations()) {

                                final boolean unknownRuleName = ifMutationHasRulesNotDefinedInSchematron(expectation,
                                                mutation.getResult().getSchematronResult());
                                final String piId = StringUtils.isBlank(mutation.getConfiguration().getMutationId())
                                                ? StringUtils.EMPTY
                                                : mutation.getConfiguration().getMutationId();
                                if (!unknownRuleName) {
                                        ...
                                        log.trace(
                                                        "mutator={} rule={} mustPass={} mustFail={} evaluatedValid={}",
                                                        mutation.getMutator().getNames(),
                                                        expectation.getRuleName(), expectation.expectValid(),
                                                        expectation.expectInvalid(), valid);
                                        ...
                                        mutation.getMutationErrorContainer().addSchematronErrorMessage(
                                                        ...
                                                        new MutationException(ErrorCode.SCHEMATRON_RULE_NOT_EXIST,
                                                                        expectation.getRuleName(),
                                                                        expectation.getSource() != null
                                                                                        ? expectation.getSource()
                                                                                        : "'no source'"));
                                        log.debug("mutator={} has rule={} which is not defined in schematron", piId,
                                                        expectation.getRuleName());
                                        log.trace(
                                                        "mutator={} rule={} mustPass={} mustFail={} evaluatedValid={}",
                                                        mutation.getMutator().getNames(),
                                                        expectation.getRuleName(), expectation.expectValid(),
                                                        expectation.expectInvalid(),
                                                        "unknown");
                        ...
                        mutation.getResult().setSchematronExpectationMatches(
                                        evaluateSchematronEnterity(schematronEnterityExpectation, mutation));
                }
        ...
        private MutationException createErrorMessage(final String ruleName, final String processingInstructionId) {
                ...
                return new MutationException(
                                ErrorCode.SCHEMATRON_RULE_WITH_INSTRUCTION_FAILED_EXPECTATION, ruleName,
                                processingInstructionId);
        }

        private Map<SchematronRuleExpectation, Boolean> evaluateSchematronEnterity(
                        final Pair<SchematronEnterity, ExpectedResult> schematronEnterityExpectation,
                ...
                for (final Map.Entry<Schematron, SchematronOutput> entry : mutation.getResult().getSchematronResult()
                                .entrySet()) {
                        final List<String> allRules = entry.getKey().getRulesIds();
                        final List<String> failedRules = entry.getValue().getFailedAsserts().stream()
                                        .map(FailedAssert::getId).toList();
                        final List<String> passedRules = allRules.stream().filter(r -> !failedRules.contains(r))
                                        .toList();
                        failedRules.forEach(f -> {
                                if (expectationNotMet(schematronEnterityExpectation, true)) {
                                        expectationsMatches.put(
                                                        new SchematronRuleExpectation(entry.getKey().getName(), f,
                                                                        expectedResult),
                                                        false);
                                        mutation.setState(State.ERROR);
                                        mutation.getMutationErrorContainer().addSchematronErrorMessage(f,
                                                        new MutationException(
                                                                        ErrorCode.SCHEMATRON_RULE_FAILED_EXPECTATION_ENTERITY));
                        ...
                        passedRules.forEach(p -> {
                                if (expectationNotMet(schematronEnterityExpectation, false)) {
                                        expectationsMatches.put(
                                                        new SchematronRuleExpectation(entry.getKey().getName(), p,
                                                                        expectedResult),
                                                        false);
                                        mutation.setState(State.ERROR);
                                        mutation.getMutationErrorContainer().addSchematronErrorMessage(p,
                                                        new MutationException(
                                                                        ErrorCode.SCHEMATRON_RULE_FAILED_EXPECTATION_ENTERITY));
        ...
        private boolean expectationNotMet(final Pair<SchematronEnterity, ExpectedResult> schematronEnterityExpectation,
                ...
                if ((enterity.equals(SchematronEnterity.ALL) && expected.equals(ExpectedResult.PASS) && failed)
                                || (enterity.equals(SchematronEnterity.ALL) && expected.equals(ExpectedResult.FAIL)
                                                && !failed)
                                || (enterity.equals(SchematronEnterity.NONE) && expected.equals(ExpectedResult.PASS)
                                                && !failed)
                                || (enterity.equals(SchematronEnterity.NONE) && expected.equals(ExpectedResult.FAIL)
                                                && failed)) {
        ...
        private boolean evaluate(final SchematronRuleExpectation expectation, MutationResult result,
                ...
                if (expectation.getSource() != null) {
                        final Optional<SchematronOutput> schematronResult = result
                                        .getSchematronResult(expectation.getSource());
                ...
                final Optional<FailedAssert> failed = targets.stream().map(SchematronOutput::getFailedAsserts)
                                .flatMap(List::stream).filter(f -> f.getId().equals(expectation.getRuleName()))
                                .findFirst();
        ...
        private void doExplicitLogging(SchematronRuleExpectation expectation,
                ...
                final Map<String, Set<String>> baseXmlSchematronFailures = mutationDocumentContext
                                .getSchematronFailures();
        ...
        /**
         * Logs if the same assertion fails multiple times in the document with mutation
         * applied.
         *
         ...
         * @param actuallyFailed          - i.e. rules failed without any mutations:
         *                                "A", "B", "C" then
         *                                rules failed with mutation: "C", "D". Actually
         *                                failed would be: "D"
         */
        private void logIfMoreThenOneSchematronRuleFailedPerMutator(SchematronRuleExpectation expectation,
                ...
                List<FailedAssert> failuresDefinedByMutator = findFailuresDefinedByMutator(actuallyFailed,
                                expectation.getRuleName());
                if (failuresDefinedByMutator.size() > 1) {
                        log.debug(
                                        "{} -> Processing instruction {} has more then one schematron rule {} failed in svrl report. Picking very first one.",
        ...
        /**
         * Log any schematron failures that occured per mutator but was not expected by
         * the mutator.
         *
         * @param expectation             - mutator expectation on XML with schematron
         * @param mutationDocumentContext - all related details to testing XML
         * @param processingInstructionId - mutation identifier in the XML
         * @param actuallyFailed          - mutation schematron failures without any
         *                                errors that occured if
         *                                any on the XML without any mutations applied.
         *                                If rule A failed on XML without mutations and
         *                                rules A, B and C
         *                                failed on mutator then result for actually
         *                                failed is B and C.
         */
        ...
        /**
         * Logs ERROR details if the same schematron rule has already failed on the XML
         * without
         ...
         * @param mutationDocumentContext   - all related details to testing XML
         * @param processingInstructionId   - mutation identifier in the XML
         * @param baseXmlSchematronFailures - failures from SVRL on XML without any
         *                                  mutators applied
         */
         ...
         *
         * @param actuallyFailed schematron assertion failures from svrl with mutation
         *                       applied
         *                       without any assertion failures from original XML
         *                       document
         *                       schematron validation without any mutations applied.
         * @param ruleName       schematron rule unique identifier from mutator
         *                       definition
         *
         * @return unexpected failures that didn't happen while testing the original XML
         *         document
         ...
         * Includes the schematron failures per mutation which hasn't occurred in the
         * XML
         * when validating the XML with schematron without any mutations. If there are
         * schematron
         * errors with mutation applied same as without any mutations applied then such
         * schematron
         * error is not included in the result.
         *
         * @param baseXmlSchematronFailures schematron failures from XML document
         *                                  without
         *                                  any mutations applied.
         *
         * @param targets                   schematron assertion failures on XML
         *                                  document with specific mutation applied.
         *
         * @return {@code targets} without {@code baseXmlSchematronFailures}
         */
        private List<FailedAssert> filterOutFailuresFoundInOriginalXml(
                        Map<String, Set<String>> baseXmlSchematronFailures,
                        Collection<SchematronOutput> targets) {
                return targets.stream()
                                ...
                                .filter(failedAssert -> !baseXmlSchematronFailures.containsKey(failedAssert.getId()) ||
                                                baseXmlSchematronFailures.containsKey(failedAssert.getId()) &&
                                                                baseXmlSchematronFailures.get(failedAssert.getId())
                                                                                .stream()
                                                                                .noneMatch(xpath -> StringUtils.equals(
                                                                                                xpath,
