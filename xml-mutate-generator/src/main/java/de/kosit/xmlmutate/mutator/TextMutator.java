...
public class TextMutator extends BaseMutator implements MutationGenerator {
    ...
    private Mutation noMutation(final MutationConfig config, final MutationDocumentContext context) {
        return create(config, context, context.getTarget().getTextContent().length());
    }

    private Collection<Mutation> maxLengthMutations(final MutationConfig config, final MutationDocumentContext context) {
        final List<Mutation> l = new ArrayList<>();
    ...
    private Collection<Mutation> minLengthMutations(final MutationConfig config, final MutationDocumentContext context) {
        final List<Mutation> l = new ArrayList<>();
    ...
    private Mutation create(final MutationConfig cloneConfig, final MutationDocumentContext cloneContext, final int length) {
        final String postfix = "length-" + length;
        cloneConfig.getProperties().put(LENGTH_PARAM, length);
        return new Mutation(cloneContext, Services.getNameGenerator().generateName(cloneContext.getDocumentName(), NAME, postfix), cloneConfig, Services.getRegistry().getMutator(NAME));
    }
}
