...
public class EmptyMutator extends BaseMutator {
    ...
    public void mutate(final MutationDocumentContext context, final MutationConfig config) {
        ...
        } else if (streamElements(target.getChildNodes()).findAny().isPresent()) {
            emptyBody(context);
        } else {
            emptyText(context);
        }
    ...
    private void emptyAttributes(final MutationDocumentContext context, final List<Object> atts) {
        final Node target = context.getTarget();
        atts.forEach(a -> {
    ...
    private void emptyText(final MutationDocumentContext context) {
        final Node target = context.getTarget();
    ...
    private void emptyBody(final MutationDocumentContext context) {
        final Node target = context.getTarget();
