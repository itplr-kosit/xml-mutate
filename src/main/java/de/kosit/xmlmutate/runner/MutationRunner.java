...
public class MutationRunner {
    ...
    public RunnerResult run() {
        ...
        final List<Pair<Path, List<Mutation>>> results = this.configuration.getDocuments().stream().map(this::process).map(MutationRunner::awaitTermination).collect(Collectors.toList());
    ...
    Future<Pair<Path, List<Mutation>>> process(final Path path) {
        return this.executorService.submit(() -> {
            ...
            if (mutations.size() == 1 && mutations.stream().anyMatch(Mutation::isErroneousOrContainsErrorMessages)) {
                ...
                final List<Mutation> sorted = mutations.stream().sorted(Comparator.comparing(e -> e.getContext().getLevel(), Comparator.reverseOrder())).collect(Collectors.toList());
    ...
    List<Mutation> parseMutations(final Document origin, final Path documentPath) {
        ...
        while (piWalker.nextNode() != null && !stopParsing) {
            ...
            if (pi.getTarget().equals("xmute")) {
                ...
                final MutationDocumentContext context = new MutationDocumentContext(pi, documentPath, this.configuration.getSavingMode());
