 ...
 */
public class MutatorDocumentContext {

    private static final Logger log = LoggerFactory.getLogger(MutatorDocumentContext.class);
    ...
    private final DocumentFragment cloneFragment;
     ...
     *
     * @param pi
     *                 the {@link ProcessingInstruction} linked to this context,
     * @param name
     *                 name of the mutation
     */
    public MutatorDocumentContext(final ProcessingInstruction pi, final String name) {

        if (name == null || name.trim().isEmpty()) {
            throw new IllegalArgumentException("Name must be set");
        }
        if (pi == null) {
            throw new IllegalArgumentException("PI must be set");
        }
        this.pi = pi;
        this.documentName = name;
        this.originalFragment = createFragment();
        this.cloneFragment = createFragment();
    }
     ...
     *
     * @param element
     *                    the DOM node which is target of the mutation
     */
    /*
     ...
     */
    public void setSpecificTarget(final Element element) {
    ...
    private DocumentFragment createFragment() {
        ...
        final Element targetElement = getTarget();
        log.trace("Found target Element={}", targetElement);
        if (targetElement != null) {
            fragment.appendChild(targetElement.cloneNode(true));
            log.trace("Fragment is={}", XmlMutateUtil.printToString(fragment, 2));
            return fragment;
        }
        return null;
    }
    ...
    public Element getParentElement() {
        final Node parentNode = this.getPi().getParentNode();
